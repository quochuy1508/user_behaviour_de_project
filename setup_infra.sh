...
SERVICE_NAME=leo-de-project
IAM_ROLE_NAME=leo-redshift

REDSHIFT_USER=leonardho
REDSHIFT_PASSWORD=Hohuy150837
REDSHIFT_PORT=5439
EMR_NODE_TYPE=m4.xlarge

echo "Creating bucket "$1""
# aws s3api create-bucket --bucket $1 --region us-east-2 --create-bucket-configuration LocationConstraint=us-east-2 --acl public-read-write --output text >> setup.log
...
echo "Attaching AWSGlueConsoleFullAccess Policy to our IAM role"
aws iam attach-role-policy --role-name $IAM_ROLE_NAME --policy-arn arn:aws:iam::aws:policy/AmazonAthenaFullAccess --output text >> setup.log

echo "Creating an AWS Redshift Cluster named "$SERVICE_NAME""
aws redshift create-cluster --cluster-identifier $SERVICE_NAME --node-type dc2.large --master-username $REDSHIFT_USER --master-user-password $REDSHIFT_PASSWORD --port $REDSHIFT_PORT --cluster-type single-node --publicly-accessible --iam-roles "arn:aws:iam::"$AWS_ID":role/"$IAM_ROLE_NAME"" >> setup.log
...
# psql -f ./redshift_setup.sql postgres://leo_user:leoP0ssword0987@leo-batch-de-project.czphpfhbid3h.us-east-2.redshift.amazonaws.com:5439/dev
psql -h $REDSHIFT_HOST -U $REDSHIFT_USER -d dev -p $REDSHIFT_PORT -f ./redshift_setup.sql
# psql -h leoredshift.czphpfhbid3h.us-east-2.redshift.amazonaws.com -U leonardho -d dev -p 5439 -f ./redshift_setup.sql
# psql "host=leo-batch-de-project.czphpfhbid3h.us-east-2.redshift.amazonaws.com user=leo_user dbname=dev port=5439 sslmode=verify-ca sslrootcert=/home/leo/Desktop/Learnspace/DataEngineer/beginner_de_project/redshift-ca-bundle.crt"
